...
internal class ShippingOrderService {
    ...
    public ShippingOrder MarkOrderAsShipped(ShippingOrder order) {
        try {
            ...
            throw new Exception($"{this.GetType().ShortDisplayName}: Failed to mark shipping order Id #{order.Id} as shipped (Shipment Id #{order.Shipment?.Id}).", ex);
        }
    }
    ...
    /// <param name="includeDeleted"></param>
    /// <returns></returns>
    public List<ShippingOrder> GetUnassignedOrdersByWarehouse(Warehouse warehouse, bool includeDeleted = false) {
        try {
    ...
    /// <param name="includeDeleted"></param>
    /// <returns></returns>
    public List<ShippingOrder> GetPackagedOrdersByWarehouse(Warehouse warehouse, bool includeDeleted = false) {
        try {
    ...
    public void CancelOrderChanges(ShippingOrder order, List<ShippingOrderProductModification> modifications, Adresse? createdAddress = null) {
        try {
            ...
            if (createdAddress != null) {
                ...
                this.parentApp.AdresseService.DeleteAdresse(createdAddress.Id);
            }
    ...
    public ShippingOrder CreateOrder(ShippingOrder order) {
        try {
            ...
            foreach (ShippingOrderProduct sop in order.ShippingOrderProducts) {
                ...
                sop.Product.ProductQuantity -= sop.Quantity;

                // Créer un ordre de restockage si la qté en stock est < 50% de la qté désirée
                if (sop.Product.ProductQuantity < sop.Product.ProductQuantityTargeted * 0.5) {
                    int quantity = sop.Product.ProductQuantityTargeted - sop.Product.ProductQuantity;
                    PurchaseOrder newPO = new PurchaseOrder(sop.Product.Id, order.SourceClient.WarehouseId, quantity);
            ...
            foreach (ShippingOrderProductModification sopModif in modifications) {
                switch (sopModif.ModificationType) {
                    case ShippingOrderProductModificationTypeEnum.Addition:
                        ...
                        // mettre à jour la quantité en stock du produit
                        sopModif.ShippingOrderProduct.Product.ProductQuantity -= sopModif.ShippingOrderProduct.Quantity;
                        _ = this.context.Products.Update(sopModif.ShippingOrderProduct.Product);

                        // Créer un ordre de restockage si la qté en stock est < 50% de la qté désirée
                        if (sopModif.ShippingOrderProduct.Product.ProductQuantity < sopModif.ShippingOrderProduct.Product.ProductQuantityTargeted * 0.5) {
                            int quantity = sopModif.ShippingOrderProduct.Product.ProductQuantityTargeted - sopModif.ShippingOrderProduct.Product.ProductQuantity;
                            PurchaseOrder newPO = new PurchaseOrder(sopModif.ShippingOrderProduct.Product.Id, order.SourceClient.WarehouseId, quantity);
                    ...
                    case ShippingOrderProductModificationTypeEnum.Modification:
                        ...
                        if (sopModif.NewQuantity > sopModif.OriginalQuantity) {
                            ...
                            // mettre à jour la quantité en stock du produit
                            sopModif.ShippingOrderProduct.Product.ProductQuantity -= sopModif.NewQuantity - sopModif.OriginalQuantity;
                            ...
                            // mettre à jour la quantité en stock du produit
                            sopModif.ShippingOrderProduct.Product.ProductQuantity += sopModif.OriginalQuantity - sopModif.NewQuantity;
                        }
                        break;
                    case ShippingOrderProductModificationTypeEnum.Removal:
